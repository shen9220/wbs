...
def save_data ( record , file_path="/Users/admin/PycharmProjects/wbs/test_case/variable/erp/zf_payee.txt" ) :
	"""保存单条数据到文件"""
	try :
		path = Path ( file_path )
...
def print_file_content ( file_path="/Users/admin/PycharmProjects/wbs/test_case/variable/erp/zf_payee.txt" ) :
	try :
		with open ( file_path , 'r' , encoding='utf-8' ) as f :
...
if __name__ == "__main__" :
	# 主URL和备用URL
	main_url = "http://wbs-test.wbscorp.com:8080/master/supplierInfo/listSupplierInfoAll"
	backup_url = "http://wbs-test.wbscorp.com:8081/master/supplierInfo/listSupplierInfoAll"
	...
	payload = json.dumps ( {
		"supplierInfoQuery" : {
			"supplierTypeList" : ["LOGISTICS_SUPPLIER"] ,
			"statusList" : ["AUDIT_PASS"]
		}
	} )
	...
	if response_data :
		# 提取数据并保存
		record = extract_fields ( response_data , index=2 )
		if record :
			save_data ( record )
			# 打印文件内容
			print_file_content ()
